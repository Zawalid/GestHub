...
function Hero() {
    ...
    return (
        <div className="relative grid grid-cols-1 min-h-[75vh] max-lg:text-center lg:grid-cols-2 pt-10 lg:px-10 2xl:px-24">
            <div className="flex my-auto flex-col pb-10 mb-10">
                ...
                <div className="flex justify-center lg:justify-start items-center gap-4 p-4">
                    <SearchInput onChange={(query) => {
                        ...
                        setSearchParams(searchParams)
                    }} query={query} placeholder={t('hero.placeholder')} />
                    <DropDown toggler={<span className={'flex items-center gap-2  border px-4 py-1 rounded-lg border-border bg-primary hover:bg-primary-hover '}>{t('hero.categories')} <IoIosArrowDown/> </span>} >
                        {
                            ['it', 'Cyber securiy', 'Secretariat'].map(e =>
                                <DropDown.Option key={e} onClick={() => setToUrl('category', e)}>{e}</DropDown.Option>)
                        }
                    </DropDown>
                </div>
           </div>
            ...
            <div className=" absolute top-0 left-0 w-full overflow-hidden leading-[0] ">
                <svg className="h-16 md:h-24 w-full" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 120" preserveAspectRatio="none">
                    ...
                    <path d="M0,0V5.63C149.93,59,314.09,71.32,475.83,42.57c43-7.64,84.23-20.12,127.61-26.46,59-8.63,112.48,12.24,165.56,35.4C827.93,77.22,886,95.24,951.2,90c86.53-7,172.46-45.71,248.8-84.81V0Z" className="fill-primary animate-pulse"></path>
                </svg>
            </div>
            <div className=" absolute rotate-180 md:top-[89.3%] top-[90.5%] left-0 w-full overflow-hidden leading-[0] ">
